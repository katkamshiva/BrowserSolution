...
class App extends Component {
  ...
  delete1 = id => {
    ...
    this.setState({len: userDetailsList.length})
  }

  render() {
    let result
    const {userDetailsList, change, len} = this.state
    console.log(len)

    const filteredList = userDetailsList.filter(
      each => each.title.toLowerCase().includes(change.toLowerCase()),
        ...
        if (len>1){
          ...
          result=<ul>
            {filteredList.map(each => (
              <BrowserItems
                initialHistoryListDetails={each}
                key={each.key}
            ...
            result=<P>No History is found</P>
        }
    )
